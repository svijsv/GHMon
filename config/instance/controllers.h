...
// controller_cfg_t and a run() function, with an optional init() function also
// possible.
//
...
static err_t fan1_run(CONTROLLER_CFG_STORAGE controller_cfg_t *cfg, controller_status_t *status) {
	...
	if (temp < TEMP_ERR_MIN || temp > TEMP_ERR_MAX) {
...
// required
//
static err_t irr1_stop_next_run_time(CONTROLLER_CFG_STORAGE controller_cfg_t *cfg, controller_status_t *status) {
	return (status->status == 0) ? 0 : (NOW() + (5 * SECONDS_PER_MINUTE));
}
static err_t irr1_stop_run(CONTROLLER_CFG_STORAGE controller_cfg_t *cfg, controller_status_t *status) {
	...
	bool stop = true;
	bool dry = (read_sensor(MOIST1_SENSOR) > MOIST_READING_DRY);
	if (status->status < 2) {
		stop = false;
		++status->status;
	}
